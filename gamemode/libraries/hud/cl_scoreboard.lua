...
function GM:HUDDrawScoreBoard()
	...
	-- surface.SetTextPos( xOffset + boardWidth - (colWidth*4) - 28,	y)	surface.DrawText("Rounds")
	...
	surface.SetTextPos( xOffset + boardWidth - (colWidth*1) + 8,	y)	surface.DrawText("CC")
	
	y = y + txHeight + 4

	local yPosition = y
	for _team,info in pairs(ScoreboardInfo) do
		...
		for index,plinfo in pairs(info.Players) do
			...
			local col = team.GetColor(plinfo.PlayerObj:Team())
			if not plinfo.PlayerObj:Alive() then
				col = Color(255, 0, 0, 155)
			end	
			...
			local t = plinfo.PlayerObj:Alive() and "" or "[RIP] "
	
			draw.SimpleText( t .. plinfo.Name, "ScoreboardText", px+1, py+1, shadowcolor )
			draw.SimpleText( t .. plinfo.Name, "ScoreboardText", px, py, textcolor )

			-- px = xOffset + boardWidth - (colWidth*4) + 8			
			-- draw.SimpleText( plinfo.Rounds, "ScoreboardText", px+1, py+1, shadowcolor )
			-- draw.SimpleText( plinfo.Rounds, "ScoreboardText", px, py, textcolor )
			...
			local money = "".. 0 .. ""		
			draw.SimpleText( money, "ScoreboardText", px+1, py+1, shadowcolor )
			draw.SimpleText( money, "ScoreboardText", px, py, textcolor )	
